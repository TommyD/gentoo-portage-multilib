...
def env_update(makelinks=1):
	...
	for x in fns:
		...
		for myspec in specials.keys():
			if myconfig.has_key(myspec):
				if myspec=="LDPATH" or myspec=="PATH":
	...
	#
	# Update prelink.conf
	if os.path.exists(root+"etc/prelink.conf"):
		#prelink.conf needs updating
		newprelink=open(root+"etc/prelink.conf","w")
		newprelink.write("# prelink.conf autogenerated by env-update; make all changes to\n")
		newprelink.write("# contents of /etc/env.d directory\n")
		newprelink.write("-l /bin\n");
		newprelink.write("-l /sbin\n");
		newprelink.write("-l /usr/bin\n");
...
auxdbkeys=['DEPEND','RDEPEND','CDEPEND','PDEPEND','REBUILD','SLOT','SRC_URI','RESTRICT','HOMEPAGE','LICENSE','DESCRIPTION','KEYWORDS','INHERITED','IUSE']
	...
	def aux_get(self,mycpv,mylist,strict=0):
		...
		if stale:
			...
			try:
				...
				print "\nportage: aux_get(): stale entry was not regenerated for",mycpv+"; exiting."
				print "!!!",e
				sys.exit(1)
			try:
